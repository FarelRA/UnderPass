...
// Configuration settings. These can be overridden by environment variables
// or URL parameters.
export let CONFIG = {
...
export function getConfig(url, env) {
    ...
    if (env.USER_ID) CONFIG.USER_ID = env.USER_ID;
    if (env.PROXY_ADDR) CONFIG.PROXY_ADDR = env.PROXY_ADDR;
    ...
    if (url.searchParams.get('proxy')) CONFIG.PROXY_ADDR = url.searchParams.get('proxy');
